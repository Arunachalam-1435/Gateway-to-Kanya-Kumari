...
class UserModel{
    ...
    public function setOrder($user_id, $product_name, $price, $order_date):bool{
        try{
            ...
            $stmt2 = $this->pdo->prepare("UPDATE business.shop SET quantity = quantity - 1 WHERE name = ? AND quantity > 0 ");
            $stmt2->execute([$product_name]);
            if($stmt2->rowCount() === 0){
                $this->pdo->rollBack();
                return false;
            }
    ...
    public function deleteOrder($user_id, $order_id, $product_name):bool{
        try{
            $stmt = $this->pdo->prepare("DELETE FROM users.orders WHERE user_id=:user_id AND order_id=:order_id");
            $stmt->bindValue(':user_id', $user_id);
            $stmt->bindValue(':order_id', $order_id);
            if($stmt->execute()){
                $stmt = $this->pdo->prepare("UPDATE business.shop SET quantity = quantity + 1 WHERE name = ?");
